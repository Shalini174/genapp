PROCESS SQL
      ...
      * WS-UNDECLARED-FIELD was not declared; statement removed.

      * STATIC ANALYSIS VIOLATION 1 (SOC1-001): ALTER verb removed;
      * replaced with inline PERFORM of existing paragraph logic.
      * OLD-PARAGRAPH only does GO TO MAINLINE-EXIT so direct PERFORM used.

      * STATIC ANALYSIS VIOLATION 2 (SOC4-003): NULL check added before SET
           IF WS-ADDR-DFHCOMMAREA EQUAL NULL
               EXEC CICS ABEND ABCODE('LGCA') NODUMP END-EXEC
           END-IF
           SET ADDRESS OF DFHCOMMAREA TO WS-ADDR-DFHCOMMAREA.

      * STATIC ANALYSIS VIOLATION 3 (SOC3-001): Zero check added before divide
           IF WS-CA-COUNT = ZERO
      ...
      * STATIC ANALYSIS VIOLATION 4 (OVF-002): D2-CUSTOMER-NUM is PIC 9(10),
      * WS-CALEN is PIC S9(4) - source has more integer digits than target.
